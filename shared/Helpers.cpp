...
auto base64Encode(unsigned char const *bytes_to_encode, unsigned int in_len)
    -> std::string {
  std::string ret;
  int i = 0;
  int j = 0;
...
auto pskHsh(const std::string &password) -> std::string {
  ...
  SHA1(data, password.length(), hash);
  return base64Encode(hash, sizeof(hash));
}

bool sendControlDescriptor(int fd,
...
bool readControlDescriptor(int fd,
                           tkm::msg::collector::Descriptor &descriptor) {
  tkm::msg::collector::Message message{};
  tkm::msg::Envelope envelope{};
  ...
  if (recv(fd, buffer, sizeof(uint64_t), MSG_WAITALL) !=
  ...
  if (recv(fd, buffer + sizeof(uint64_t), messageSize, MSG_WAITALL) !=
      messageSize) {
    return false;
  }
