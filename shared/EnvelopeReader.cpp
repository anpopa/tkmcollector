...
EnvelopeReader::EnvelopeReader(int fd) : IAsyncEnvelope("EnvelopeReader", fd) {}

auto EnvelopeReader::next(tkm::msg::Envelope &envelope)
    -> IAsyncEnvelope::Status {
  std::scoped_lock lk(m_mutex);

  auto retVal =
      read(m_fd, m_buffer + m_bufferOffset, sizeof(m_buffer) - m_bufferOffset);
  if (retVal < 0) {
  ...
  if ((messageSize + sizeof(uint64_t)) < m_bufferOffset) {
    memmove(m_buffer, m_buffer + (sizeof(uint64_t) + messageSize),
