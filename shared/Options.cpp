...
auto Options::getFor(Key key) -> string {
  ...
  case Key::DBType:
    if (hasConfigFile()) {
      const optional<string> prop =
          m_configFile->getPropertyValue("database", -1, "Type");
  ...
  case Key::DBUserPassword:
    if (hasConfigFile()) {
      ...
      return prop.value_or(
          tkmDefaults.getFor(Defaults::Default::DBUserPassword));
    }
  ...
  case Key::DBServerAddress:
    if (hasConfigFile()) {
      ...
      return prop.value_or(
          tkmDefaults.getFor(Defaults::Default::DBServerAddress));
    }
  ...
  case Key::RuntimeDirectory:
    if (hasConfigFile()) {
      ...
      return prop.value_or(
          tkmDefaults.getFor(Defaults::Default::RuntimeDirectory));
    }
