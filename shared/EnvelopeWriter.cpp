...
EnvelopeWriter::EnvelopeWriter(int fd) : IAsyncEnvelope("EnvelopeWriter", fd) {}

auto EnvelopeWriter::send(const tkm::msg::Envelope &envelope)
...
auto EnvelopeWriter::flushInternal() -> bool {
  ...
  while ((sizeToSend > 0) && status) {
    auto retVal =
        write(m_fd, m_buffer + (m_bufferOffset - sizeToSend), sizeToSend);
    if (retVal < 0) {
