...
UDSServer::UDSServer() : Pollable("UDSServer") {
  ...
  lateSetup(
      [this]() {
        ...
        if (!readControlDescriptor(clientFd, descriptor)) {
          logWarn() << "Control client " << clientFd
        ...
        std::shared_ptr<ControlClient> client =
            std::make_shared<ControlClient>(clientFd);
        client->enableEvents();

        return true;
      },
      m_sockFd, bswi::event::IPollable::Events::Level,
...
void UDSServer::enableEvents() { CollectorApp()->addEventSource(getShared()); }

UDSServer::~UDSServer() { static_cast<void>(stop()); }
...
void UDSServer::start() {
  ...
  if (bind(m_sockFd, (struct sockaddr *)&m_addr, sizeof(struct sockaddr_un)) !=
      -1) {
    // We are ready for events only after start
