...
Connection::Connection() : Pollable("Connection") {
      ...
      [this]() {
        ...
        do {
          ...
          case tkm::msg::collector::Message_Type_SetSession: {
            ...
            rq.bulkData =
                std::make_any<tkm::msg::collector::SessionInfo>(sessionInfo);
          ...
          case tkm::msg::collector::Message_Type_Status: {
            Dispatcher::Request rq{.action =
                                       Dispatcher::Action::CollectorStatus};
          ...
          case tkm::msg::collector::Message_Type_DeviceList: {
            ...
            rq.bulkData =
                std::make_any<tkm::msg::collector::DeviceList>(deviceList);
...
void Connection::enableEvents() { ControlApp()->addEventSource(getShared()); }
...
auto Connection::connect() -> int {
  ...
  if (::connect(m_sockFd, (struct sockaddr *)&m_addr,
