...
static auto doConnect(const shared_ptr<Dispatcher> &mgr,
                      const Dispatcher::Request &rq) -> bool;
static auto doSendDescriptor(const shared_ptr<Dispatcher> &mgr,
                             const Dispatcher::Request &rq) -> bool;
static auto doRequestSession(const shared_ptr<Dispatcher> &mgr,
                             const Dispatcher::Request &rq) -> bool;
static auto doSetSession(const shared_ptr<Dispatcher> &mgr,
                         const Dispatcher::Request &rq) -> bool;
...
static auto doTerminateCollector(const shared_ptr<Dispatcher> &mgr,
                                 const Dispatcher::Request &rq) -> bool;
static auto doCollectorStatus(const shared_ptr<Dispatcher> &mgr,
                              const Dispatcher::Request &rq) -> bool;
static auto doDeviceList(const shared_ptr<Dispatcher> &mgr,
                         const Dispatcher::Request &rq) -> bool;

void Dispatcher::enableEvents() { ControlApp()->addEventSource(m_queue); }
...
auto Dispatcher::requestHandler(const Request &request) -> bool {
  ...
  case Dispatcher::Action::StartDeviceSession:
    return doStartDeviceSession(getShared(), request);
  case Dispatcher::Action::StopDeviceSession:
    return doStopDeviceSession(getShared(), request);
  case Dispatcher::Action::TerminateCollector:
    return doTerminateCollector(getShared(), request);
...
static auto doConnect(const shared_ptr<Dispatcher> &mgr,
                      const Dispatcher::Request &) -> bool {
  Dispatcher::Request rq;

  if (ControlApp()->getConnection()->connect() < 0) {
    std::cout << "Connection to tkm-collector failed" << std::endl;
...
static auto doSendDescriptor(const shared_ptr<Dispatcher> &mgr,
  ...
  if (!sendControlDescriptor(ControlApp()->getConnection()->getFD(),
                             descriptor)) {
...
static auto doSetSession(const shared_ptr<Dispatcher> &mgr,
  ...
  const auto &sessionInfo =
      std::any_cast<tkm::msg::collector::SessionInfo>(rq.bulkData);
...
static auto doQuit(const shared_ptr<Dispatcher> &, const Dispatcher::Request &)
    -> bool {
  std::cout << std::flush;
  exit(EXIT_SUCCESS);
}
...
static auto doAddDevice(const shared_ptr<Dispatcher> &mgr,
  ...
  logDebug() << "Request add device for: "
             << rq.args.at(Defaults::Arg::DeviceName);
  return ControlApp()->getConnection()->writeEnvelope(requestEnvelope);
}
...
static auto doRemoveDevice(const shared_ptr<Dispatcher> &mgr,
  ...
  deviceData.set_id(std::stoul(rq.args.at(Defaults::Arg::DeviceId)));
  requestMessage.mutable_data()->PackFrom(deviceData);
  ...
  logDebug() << "Request remove device for: "
             << rq.args.at(Defaults::Arg::DeviceName);
  return ControlApp()->getConnection()->writeEnvelope(requestEnvelope);
}
...
static auto doConnectDevice(const shared_ptr<Dispatcher> &mgr,
  ...
  logDebug() << "Request connect device for: "
             << rq.args.at(Defaults::Arg::DeviceName);
  return ControlApp()->getConnection()->writeEnvelope(requestEnvelope);
}
...
static auto doDisconnectDevice(const shared_ptr<Dispatcher> &mgr,
  ...
  logDebug() << "Request disconnect device for: "
             << rq.args.at(Defaults::Arg::DeviceName);
  return ControlApp()->getConnection()->writeEnvelope(requestEnvelope);
}
...
static auto doStartDeviceSession(const shared_ptr<Dispatcher> &mgr,
  ...
  requestMessage.set_id("StartCollectDevice");
  requestMessage.set_type(tkm::msg::collector::Request_Type_StartDeviceSession);
  if (rq.args.count(Defaults::Arg::Forced)) {
    if (rq.args.at(Defaults::Arg::Forced) ==
        tkmDefaults.valFor(Defaults::Val::True)) {
      requestMessage.set_forced(tkm::msg::collector::Request_Forced_Enforced);
  ...
  logDebug() << "Request start collect device for: "
             << rq.args.at(Defaults::Arg::DeviceName);
  return ControlApp()->getConnection()->writeEnvelope(requestEnvelope);
}
...
static auto doStopDeviceSession(const shared_ptr<Dispatcher> &mgr,
  ...
  requestMessage.set_id("StartCollectDevice");
  requestMessage.set_type(tkm::msg::collector::Request_Type_StopDeviceSession);
  if (rq.args.count(Defaults::Arg::Forced)) {
    if (rq.args.at(Defaults::Arg::Forced) ==
        tkmDefaults.valFor(Defaults::Val::True)) {
      requestMessage.set_forced(tkm::msg::collector::Request_Forced_Enforced);
  ...
  logDebug() << "Request stop collect device for: "
             << rq.args.at(Defaults::Arg::DeviceName);
  return ControlApp()->getConnection()->writeEnvelope(requestEnvelope);
}
...
static auto doTerminateCollector(const shared_ptr<Dispatcher> &mgr,
  ...
  requestMessage.set_id("TerminateCollector");
  requestMessage.set_type(tkm::msg::collector::Request_Type_TerminateCollector);
  if (rq.args.count(Defaults::Arg::Forced)) {
    if (rq.args.at(Defaults::Arg::Forced) ==
        tkmDefaults.valFor(Defaults::Val::True)) {
      requestMessage.set_forced(tkm::msg::collector::Request_Forced_Enforced);
  ...
  logDebug() << "Request terminate collector";
...
static auto doCollectorStatus(const shared_ptr<Dispatcher> &mgr,
  ...
  const auto &requestStatus =
      std::any_cast<tkm::msg::collector::Status>(rq.bulkData);
  const auto &statusText =
      (requestStatus.what() == tkm::msg::collector::Status_What_OK)
          ? tkmDefaults.valFor(Defaults::Val::StatusOkay)
          : tkmDefaults.valFor(Defaults::Val::StatusError);

  logDebug() << "Collector status(" << requestStatus.requestid()
             << "): " << statusText << " Reason: " << requestStatus.reason();
  ...
  std::cout << "--------------------------------------------------"
            << std::endl;
  std::cout << "Status: " << statusText << " Reason: " << requestStatus.reason()
            << std::endl;
  std::cout << "--------------------------------------------------"
            << std::endl;
...
static auto doDeviceList(const shared_ptr<Dispatcher> &mgr,
  ...
  for (int i = 0; i < deviceList.device_size(); i++) {
    ...
    case tkm::msg::collector::DeviceData_State_Disconnected:
      ...
      break;
    case tkm::msg::collector::DeviceData_State_SessionActive:
      std::cout << "SessionActive" << std::endl;
      break;
    case tkm::msg::collector::DeviceData_State_SessionInactive:
      std::cout << "SessionInactive" << std::endl;
      break;
